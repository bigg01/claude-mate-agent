...
- `container/guardrails.py` is a self-contained module with five independently-toggleable controls — cost cap (pre-flight refuse + post-task warn), input scrubbing, output scrubbing, workspace `.claudeignore` writer, per-persona intent denylist. Each family is a no-op when its env flag is unset; the chart's `claude-mate-agent.guardrailsEnv` helper (in `_helpers.tpl`) renders no env vars when `guardrails.enabled: false`, so disabled deployments pay zero startup cost. Wired into `app.py`'s `run_once()` at three points: before `_build_claude_cmd` (cost / intent / input / workspace) and after `_parse_claude_output` (output scrub + cost record). Helm helper is included in both `templates/deployment.yaml` and `templates/sandbox-job.yaml` so static and sandbox modes share guardrail behaviour. Example: `examples/guardrails/values.yaml`. Tests: `container/tests/test_guardrails.py` (27 cases). Spec: `requirement.md` §28. Docs: `docs/guardrails.md`. Pattern groups out-of-the-box: `api-keys` (Anthropic/OpenAI/AWS/Google/GitHub/GitLab/Slack), `credentials` (PEM private keys), `pii` (SSN, credit cards), `network` (RFC 1918). Modes: `redact` (mask matches, continue) and `block` (refuse, emit `guardrail_blocked`).
